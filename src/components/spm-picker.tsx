...
export default function SpmPicker({
  value,
  onValueChange,
  defaultValue,
}: {
  ...
  const spmSelected = api.spm.getById.useQuery(selected!, {
    enabled: !!selected,
  ...
  const spm = api.spm.getAll.useQuery(
    { search: searchValue, haveSp2d: false },
    { placeholderData: keepPreviousData },
  );
    ...
    <Dialog>
      <DialogTrigger asChild>
        ...
        >
          {selected !== undefined && (
            <div>
              {spmSelected.isSuccess && spmSelected.data && (
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col text-left">
                    <span className="line-clamp-1">
                      {spmSelected.data.noDokumen}
                    </span>
                    <span className="line-clamp-1 text-xs text-slate-500">
                      {formatTanggal(spmSelected.data.tglDokumen)}
                    </span>
                  </div>
                </div>
              )}
              {spmSelected.isLoading && (
                <div className="flex items-center gap-3">
                  <Spinner />
                </div>
              )}
      ...
      <DialogContent className="max-w-5xl">
        <DialogHeader>
          <DialogTitle>Pilih SPM</DialogTitle>
        </DialogHeader>
        <Input
