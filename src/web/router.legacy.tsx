...
const importedRoutesEager = import.meta.glob('@/app/**/(layout|not-found|middleware).tsx', { eager: true })
...
function middlewareResolver(objRoute: RouteType) {
    const listOfMiddlewares = routesList
        ...
        .filter((route) => objRoute.segments.join('/').includes(route.segments.join('/')) && objRoute.key !== route.key)
    ...
    const listOfLayouts = routesList
        ...
        .filter((route) => {
            if (objRoute.type === 'middleware')
                return (
                    objRoute.segments.slice(0, -1).join('/').includes(route.segments.join('/')) &&
                    objRoute.key !== route.key
                )

            return objRoute.segments.join('/').includes(route.segments.join('/')) && objRoute.key !== route.key
        })
        .sort((a, b) => a.segments.length - b.segments.length)
        ...
        return {
            ...
            element: (
                <React.Suspense fallback={<Loading />}>
                    <motion.div key={route.key} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                        <route.element />
                    </motion.div>
                </React.Suspense>
            ),
