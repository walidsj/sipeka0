                ...
                <div className="flex w-full items-center justify-between">
                    <Link to="/" className="flex h-20 flex-shrink-0 items-center gap-4">
                        <img
                            ...
                            className={cn(
                                ...
                                pathname.startsWith('__DASHBOARD_PREFIX__') && 'hidden h-6',
                                pathname.startsWith('__CLIENT_PREFIX__') && 'hidden h-6'
                            )}
                        />
                        ...
                        {pathname.startsWith('__CLIENT_PREFIX__') && (
                            <motion.div
                                ...
                                className="flex h-8 items-center md:border-l md:px-4"
                            >
                                <img src="/images/logo-myatma.svg" alt="Logo MyAtma" className="block h-9 w-auto" />
                            </motion.div>
                        )}
                    </Link>
                    <div className="flex items-center">
                        ...
                        {!auth.user ? (
                            <React.Fragment>
                                {auth.isLoading ? (
                                    <Button disabled variant="ghost" className="h-16 gap-3">
                                    ...
                                    <Button variant="ghost" asChild className="h-16">
                                        <Link to="/login">
                            ...
                            <React.Fragment>
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <Button variant="ghost" className="h-16 gap-3">
                                            <Avatar>
                                                <AvatarImage
                                                    src={
                                                        auth.user.image
                                                            ? `/api/storage/files/user-image/${auth.user.image}`
                                                            : `https://ui-avatars.com/api/?name=${auth.user.nama}&background=3b82f6&color=fff`
                                                    }
                                                />
                                                <AvatarFallback>
                                                    <HiOutlineUser />
                                            ...
                                            <div className="hidden text-left lg:block">
                                                ...
                                                <div className="block text-xs font-normal text-slate-400">
                                                    ...
                                                    {auth.user.pegawai?.pengelolaBlud.map((blud, index) => (
                                                        <div className="text-xs font-normal text-slate-400" key={index}>
                                                            {blud.role}
                                                        </div>
                                                    ))}
                                                </div>
                                    ...
                                    <DropdownMenuContent align="start">
                                        ...
                                        <Link to="/profil/ganti-password" className="w-full">
