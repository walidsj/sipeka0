...
export default function MonitoringTable() {
  const rbaMonitoring = api.dba.getRbaBelanjaMonitoring.useQuery(undefined, {
    suspense: true,
  });
      ...
      <Table className="border-collapse border">
        <TableHeader>
          ...
          <TableRow>
            ...
            <TableHead className="border text-center">Harga Satuan</TableHead>
            <TableHead className="border text-center">Keuangan</TableHead>
          ...
          {rbaMonitoring.data?.aktivitas.map((aktivitas) => (
            <React.Fragment key={aktivitas.id}>
              <TableRow className="bg-blue-50 font-semibold hover:bg-blue-100">
                <TableCell className="border">{aktivitas.kode}</TableCell>
                ...
                <TableCell className="border text-right">
                  {aktivitas.rincianRbaBelanja
                    .reduce(
                      (acc, item) =>
                        acc + Number(Number(item.volume) * Number(item.harga)),
                      0,
                    )
                    ...
                    Number(
                      (Number(
                        ...
                        aktivitas.rincianRbaBelanja.reduce(
                          (acc, item) =>
                            acc +
                            Number(Number(item.volume) * Number(item.harga)),
                          0,
                        )) *
                        100,
                    ).toLocaleString("id-ID", {
              ...
              {Object.keys(
                ...
                <React.Fragment key={key}>
                  <TableRow className="bg-yellow-50 hover:bg-yellow-100">
                    ...
                    <TableCell className="border font-semibold" colSpan={4}>
                      {
                        realisasiMonitoring.data?.find((item) => {
                          return item.kodeRekening === key;
                        })?.rekening?.uraian
                      }
                    </TableCell>
                  ...
                  {lodash
                    ...
                    [key].map((rincian) => {
                      totalPagu =
                        totalPagu +
                        Number(Number(rincian.volume) * Number(rincian.harga));

                      totalRealisasi =
                        totalRealisasi +
                        Number(
                          realisasiMonitoring.data?.find((item) => {
                            return item.id === rincian.rabId;
                          })?.jumlah,
                        );
                      ...
                      return (
                        <TableRow
                          ...
                          className={cn(
                            Number(
                              Number(rincian.volume) * Number(rincian.harga) -
                                Number(
                                  realisasiMonitoring.data?.find((item) => {
                                    return item.id === rincian.rabId;
                                  })?.jumlah,
                                ),
                            ) < 0 && "bg-red-50 text-red-500 hover:bg-red-100",
                          )}
                        >
                          <TableCell className="border">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="start">
                                <Link
                                  to="/anggaran/monitoring/realisasi-belanja/$rincianRbaBelanjaId/detail-belanja"
                                  params={{
                                    rincianRbaBelanjaId: String(rincian.id),
                                  }}
                                >
                                  <DropdownMenuItem>
                                    <FiList />
                                ...
                                <Link
                                  to="/anggaran/monitoring/realisasi-belanja/$rincianRbaBelanjaId/edit"
                                  params={{
                                    rincianRbaBelanjaId: String(rincian.id),
                                  }}
                                >
                          ...
                          <TableCell className="border">
                            <p className="font-medium">{rincian.rab?.uraian}</p>
                            {rincian.rab?.spesifikasi && (
                          ...
                          <TableCell className="border text-center">
                            {Number(rincian.volume).toLocaleString("id-ID")}
                          </TableCell>
                          <TableCell className="border text-center">
                            {rincian.satuan}
                          </TableCell>
                          <TableCell className="border text-right">
                            {Number(rincian.harga).toLocaleString("id-ID")}
                          </TableCell>
        ...
        <TableFooter>
          <TableRow className="bg-gray-50">
            <TableCell className="border text-right font-semibold" colSpan={5}>
              Total
            </TableCell>
            <TableCell className="border text-right font-semibold">
            ...
            <TableCell className="border text-right font-semibold">
              {Number((totalRealisasi / totalPagu) * 100).toLocaleString(
                "id-ID",
                {
                  maximumFractionDigits: 2,
                },
            ...
            <TableCell className="border text-right font-semibold">
              {Number(totalPagu - totalRealisasi).toLocaleString("id-ID")}
            </TableCell>
          </TableRow>
        </TableFooter>
      </Table>
