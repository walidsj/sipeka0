  ...
  importCsv: pengelolaProcedure([
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const rekeningBankIds = await ctx.db
        ...
        .where(inArray(rekeningBankTable.noRekening, noRekeningListUnique));

      const inputData = data.map((item) => {
        const rekeningBankId = rekeningBankIds.find(
          (rekeningBank) => rekeningBank.noRekening === item.noRekening,
        )?.id;
        if (rekeningBankId) {
          return {
            rekeningBankId,
            ...
            debet: String(
              Number(item.debet.replace(/\./g, "").replace(/,/g, ".")).toFixed(
                2,
              ),
            ),
            kredit: String(
              Number(item.kredit.replace(/\./g, "").replace(/,/g, ".")).toFixed(
                2,
              ),
            ),
